    ...
    <div className={`card ${project.status === 'completed' ? styles.projectCompletedBorder : ''}`}>
      <div className={styles.stageHeaderWrapper}>
        <div className={styles.stageTitleBlock}>
          ...
          {project.status === 'cancelled' && <span className={styles.overdue}>Отменен</span>}
        </div>
      </div>
      ...
      {user.role === 'Прораб' && project.planStatus !== 'approved' && project.planStatus !== 'pending_approval' && project.status === 'active' && project.ProjectStages?.length > 0 && (
        <button onClick={onPlanSubmit} className={styles.submitPlanButton}>ОТПРАВИТЬ ПЛАН НА УТВЕРЖДЕНИЕ</button>
      )}
      {user.role === 'Заказчик' && project.planStatus === 'pending_approval' && project.status === 'active' && (
      ...
      {user.role === 'Заказчик' && project.status === 'active' && project.planStatus === 'approved' && allStagesApproved && project.ProjectStages?.length > 0 && (
